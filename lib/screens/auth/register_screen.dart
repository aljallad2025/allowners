...
class _RegisterScreenState extends ConsumerState<RegisterScreen> {
  ...
  Future<void> _register() async {
    ...
    if (_passwordController.text.length < 6) {
      setState(() => _errorMessage = AppStrings.t(isArabic, 'password_too_short'));
      return;
    }
    setState(() {
      _loading = true;
    ...
    try {
      ...
      if (supabase.auth.currentSession != null) {
        Navigator.of(context).pushAndRemoveUntil(
          MaterialPageRoute(
            builder: (_) =>
                _selectedRole == UserRole.owner ? const OwnerNavigationScreen() : const MainNavigationScreen(),
          ),
          (route) => false,
        );
      } else {
        // يحتاج تأكيد بريد إلكتروني قبل الدخول
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppStrings.t(isArabic, 'check_email_confirm'))),
        );
        Navigator.of(context).pop();
      }
    } catch (e) {
      setState(() => _errorMessage = AppStrings.t(isArabic, 'register_failed'));
    } finally {
