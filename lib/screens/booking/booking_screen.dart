...
class _BookingScreenState extends ConsumerState<BookingScreen> {
  ...
  Future<void> _confirmBooking() async {
    ...
    final roomPrice = widget.hotel.pricePerNight * _nights;
    final taxes = roomPrice * 0.15;
    final total = roomPrice + taxes;
    final checkOut = _checkIn.add(Duration(days: _nights));
    ...
    try {
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (_) => BookingSuccessScreen(hotel: widget.hotel)),
      );
    } catch (e) {
      if (!mounted) return;
                      ...
                      children: [
                        Expanded(
                          child: _InfoBox(
                            ...
                            value: '${_checkIn.day}/${_checkIn.month}/${_checkIn.year}',
                          ),
                        ),
                        const SizedBox(width: AppDimens.sm),
                        Expanded(
                          child: _InfoBox(
                            ...
                            value:
                                '${_checkIn.add(Duration(days: _nights)).day}/${_checkIn.add(Duration(days: _nights)).month}/${_checkIn.add(Duration(days: _nights)).year}',
                          ),
                        ),
                      ],
                    ),
