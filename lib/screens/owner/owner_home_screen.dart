...
class _OwnerHomeScreenState extends ConsumerState<OwnerHomeScreen> {
  ...
  void initState() {
    ...
    // نحمّل الصلاحيات الفعلية أول ما توصل الشاشة (بتتحدث تلقائياً بعد تسجيل الدخول كمان)
    Future.microtask(() => ref.read(permissionsProvider.notifier).load());
  }

  void _reload() {
    setState(() => _future = _service.getDashboard());
    ref.read(permissionsProvider.notifier).load();
  }
    ...
    final quickLinks = <_QuickLink>[
      ...
      if (perms.can('units', 'view'))
        _QuickLink(
          ...
          title: AppStrings.t(isArabic, 'nav_units'),
      ...
      if (perms.role == 'booking_agent') ...[
        ...
        _QuickLink(
          ...
          title: AppStrings.t(isArabic, 'new_booking'),
          onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const OwnerAgentNewBookingScreen())),
        ),
      ],
    ];
